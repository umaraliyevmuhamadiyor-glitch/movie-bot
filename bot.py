...
def process_broadcast(message):
    ...
    bot.send_message(message.chat.id, f"📢 Reklama {len(users)} foydalanuvchiga yuborilmoqda...")
...
def show_admin_settings(call):
    keyboard = types.InlineKeyboardMarkup()
    btn1 = types.InlineKeyboardButton("🔄 Ma'lumotlarni yangilash", callback_data="admin_refresh")
    btn2 = types.InlineKeyboardButton("📊 DB holati", callback_data="admin_db_status")
...
def backup_database(call):
    try:
        # Ma'lumotlar bazasini yopib qayta ochamiz
        db.close()
        time.sleep(1)
        global db
        db = init_db()
        
        bot.answer_callback_query(call.id, "✅ Ma'lumotlar bazasi qayta yuklandi!")
    except Exception as e:
        bot.answer_callback_query(call.id, "❌ Backupda xatolik!")
...
if __name__ == '__main__':
    ...
    print("👨‍💼 Admin panel faollashtirildi!")
